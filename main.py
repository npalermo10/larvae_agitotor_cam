...
b4_shake_save_directory = "/home/pi/larvae_agitator_cam/pics/shake"
# if not os.path.exists(b4_shake_save_directory):
    # os.makedirs(b4_shake_save_directory)
...
# GPIO.setup(shake_pin, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)
...
try:
    while running:
        curr_t = t.time()
        if int((curr_t - start_t)%3600) == sec_btw_pics and (curr_t -last_main_cycle_photo_t) >= 1:
